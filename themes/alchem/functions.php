...
function alchem_scripts() {
   ...
   if( $body_font ){
	wp_enqueue_style( 'google-fonts-'.sanitize_title($body_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.useso.com/css?family=' . str_replace(' ','+',$body_font), array(), '' );
   }
   if( $menu_font ){
	wp_enqueue_style( 'google-fonts-'.sanitize_title($menu_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.useso.com/css?family=' . str_replace(' ','+',$menu_font), array(), '' );
   }
   if( $headings_font ){
	wp_enqueue_style( 'google-fonts-'.sanitize_title($headings_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.useso.com/css?family=' . str_replace(' ','+',$headings_font), array(), '' );
   }
    if( $footer_headings_font ){
	wp_enqueue_style( 'google-fonts-'.sanitize_title($footer_headings_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.useso.com/css?family=' . str_replace(' ','+',$footer_headings_font), array(), '' );
   }
   if( $button_font ){
	wp_enqueue_style( 'google-fonts-'.sanitize_title($button_font), 'http' . ( ( is_ssl() ) ? 's' : '' ) . '://fonts.useso.com/css?family=' . str_replace(' ','+',$button_font), array(), '' );
   }
					
	$google_fonts = alchem_option('google_fonts');
    if( trim($google_fonts) !='' ){
	$google_fonts = str_replace(' ','+',trim($google_fonts));
	wp_enqueue_style('alchem-google-fonts', esc_url('//fonts.useso.com/css?family='.$google_fonts), false, '', false );
	}
	
	$home_style = absint(alchem_option('home_style'));
	if( $home_style == '1' ){
		wp_enqueue_style('alchem-google-fonts-cuprum-Playfair-Display-Dancing-Script', esc_url('//fonts.useso.com/css?family=Cuprum|Playfair+Display|Dancing+Script'), false, '', false );
		}
	if( $home_style == '2' ){
		wp_enqueue_style('alchem-google-fonts-poiret-one-Dancing-Script', esc_url('//fonts.useso.com/css?family=Poiret+One|Dancing+Script'), false, '', false );
   }
//    wp_enqueue_style( 'alchem-Open-Sans', esc_url('//fonts.useso.com/css?family=Open+Sans:300,400,700'), false, '', false );
